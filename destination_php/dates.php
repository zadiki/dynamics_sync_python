...
// Configuration for the Report Table
$tableName = "Athsalesordersummaryreport";
$allowedColumns = [
    'DESCRIPTION', 
    'FROMDATE', 
    'TODATE'
...
function handleGet($pdo, $tableName, $allowedColumns) {
    ...
    if (!empty($_GET)) {
        ...
        foreach ($_GET as $key => $value) {
            // Case-insensitive check for allowed columns
            $upperKey = strtoupper($key);
            if (in_array($upperKey, array_map('strtoupper', $allowedColumns))) {
                $conditions[] = "$upperKey = ?";
                $params[] = $value;
            }
        }
...
function handlePost($pdo, $tableName, $allowedColumns) {
    ...
    if (!$payload || !is_array($payload)) {
        ...
        echo json_encode(["success" => false, "error" => "Invalid JSON array"]);
        return;
    }
    ...
    try {
        ...
        // 1. Wipe existing report entries
        ...
        foreach ($payload as $row) {
            ...
            foreach ($allowedColumns as $col) {
                // We use null if the key is missing to avoid "Undefined index" notices
                $values[] = isset($row[$col]) ? $row[$col] : (isset($row[strtolower($col)]) ? $row[strtolower($col)] : null);
            }
            $stmt->execute($values);
        }
